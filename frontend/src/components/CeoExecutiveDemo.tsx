...
const CeoExecutiveDemo = () => {
  ...
  return (
    <div className="ce-workspace">
      ...
      {modal === 'start' && <CeoDialog title="Start your executive workday" description="Set a calm intention before the meetings and decisions begin." icon={<Play size={18}/>} onClose={() => setModal(null)}><div className="ce-simple-dialog"><div><span>Today’s leadership focus</span><strong>Clear delivery blockers and approve the two agent outputs.</strong><p>This is visible only as your focus, not as an employee productivity score.</p></div><footer><button type="button" onClick={() => setModal(null)}>Cancel</button><button type="button" onClick={beginDay}><Play size={14}/> Start workday</button></footer></div></CeoDialog>}
