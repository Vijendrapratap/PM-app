...
const RoleHeader = ({ eyebrow, title, description, action }: { eyebrow: string; title: string; description: string; action: string }) => (
  <header className="role-hero"><div><span>{eyebrow}</span><h1>{title}</h1><p>{description}</p></div><div className="role-hero-side"><small>{dateLabel()}</small><Link to="/projects">{action}<ArrowUpRight size={14}/></Link></div></header>
);
...
export const ProjectManagerDashboard = (data: DashboardData) => (
  <div className="role-dashboard pm-dashboard animate-fade-in">
    <RoleHeader eyebrow="Project manager workspace" title={`${data.name.split(' ')[0]}, decisions come first`} description="Approve agent work, resolve delivery risk, and make sure every task has a clear owner." action="Open all projects"/>
...
export const TechLeadDashboard = (data: DashboardData & { taskGroups: Array<{ project: Project; tasks: ProjectTask[] }> }) => {
  ...
  return <div className="role-dashboard lead-dashboard animate-fade-in">
    <RoleHeader eyebrow="Tech lead workspace" title={`${data.name.split(' ')[0]}, keep delivery reviewable`} description="Focus on technical review, dependencies, blockers, and whether the next release is genuinely ready." action="Open delivery board"/>
...
export const CeoDashboard = (data: DashboardData & { totalProjects: number; completedProjects: number; teamSize: number }) => {
  ...
  return <div className="role-dashboard ceo-dashboard animate-fade-in">
    <RoleHeader eyebrow="CEO portfolio" title={`${data.name.split(' ')[0]}, see the company clearly`} description="Portfolio movement, delivery confidence, people pressure and agent governance—without dropping into every task." action="Explore portfolio"/>
