...
const AgentStudio = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([agentWorkflowApi.definitions(), agentWorkflowApi.status()])
      .then(([items, status]) => { setDefinitions(items); setSelectedId(items[0]?._id || ''); setProviderStatus(status); })
      .catch((loadError) => setError(getErrorMessage(loadError, 'Agent Studio is not available yet.')))
